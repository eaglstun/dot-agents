...
WebView shell (see `AGENTS.md` and the `headset` skill). Your job is to return the **real,
current Apple API answer** — grounded in actual `developer.apple.com` docs — not a half-
remembered one.

## Read these first (every task)

1. `AGENTS.md` at the repo root — the platform constraints, the iOS gotchas (motion permission
...
- **Note availability.** ARKit symbols carry `iOS <n>.0+` / deprecation markers — report them.
  The primary user is on a recent iPhone (iPhone 17 Pro per `AGENTS.md`), so flag anything that
