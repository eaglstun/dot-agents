...
1. **Read the repo's `AGENTS.md` or `README.md`** at the root — the architecture and
...
**Crosslinks (shared-shelf convention):** each shelf is a skill's `references/` dir, so `[[shelf:file]]` resolves to `~/.agents/skills/<shelf>/references/<file>.md`. Link across shelves as `[[shelf:file]]` (e.g. `[[apple-silicon:mps-matrix-multiplication]]`) in a `### See also` footer, and give every link a reason ("CUDA twin of…", "contradicts…"). Before writing a new GPU topic, check `~/.agents/skills/cuda/references/gpu-rosetta.md` — the CUDA↔Metal concept map and convention spec. Keep MSL `[[attribute]]` syntax inside code spans so it does not parse as a link; `bash ~/.agents/scripts/audit-crosslinks.sh` verifies all links.
