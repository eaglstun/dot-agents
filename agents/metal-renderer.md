...
description: >-
  ...
  `~/.agents/skills/apple-metal/references/` doc library.
...
1. `AGENTS.md` at the repo root — platform constraints, the clicker input model, `lensShift` /
...
**Project reference docs — consult and contribute (with sources).** Offline reference
libraries live in the skills under `~/.agents/skills/` (`apple-arkit/references/`, `threejs/references/`,
`apple-arkit/native-models/`, and `apple-metal/references/` for renderer-relevant Metal/MSL/graphics notes). Consult them before the open web. When
you have to dig out something durable to land an effect — an MSL gotcha, a blend/pipeline recipe,
a depth detail — capture it as `~/.agents/skills/apple-metal/references/<topic>.md` 
