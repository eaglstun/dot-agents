...
description: >-
  ...
  `metal-renderer`), but it MAY save source-cited notes to `~/.agents/skills/apple-metal/references/`.
...
1. `AGENTS.md` at the repo root — platform constraints, the clicker, `lensShift`/IPD. The
...
Curated, offline reference libraries live in the skills under `~/.agents/skills/`, tailored to
this project's constraints — **consult them before the open web**: `apple-arkit/references/` (ARKit / CoreMotion
/ camera / scene-depth), `threejs/references/` (the web path), `apple-arkit/native-models/` (RealityKit — the path this
app does NOT use). For the renderer there's `apple-metal/references/` — renderer-relevant Metal / MSL / graphics
notes, the graphics-side companion to the compute-scoped global `apple-silicon` skill.
...
phone-GPU cost you reasoned through — capture it as `~/.agents/skills/apple-metal/references/<topic>.md` so the
...
`~/.agents/skills/apple-metal/references/` when the research is worth keeping; note it if you did.

**Crosslinks (shared-shelf convention):** each shelf is a skill's `references/` dir, so `[[shelf:file]]` resolves to `~/.agents/skills/<shelf>/references/<file>.md`. Link across shelves as `[[shelf:file]]` (e.g. `[[apple-silicon:simd-group-functions]]`) in a `### See also` footer, and give every link a reason ("CUDA twin of…", "contradicts…"). Before writing a new GPU topic, check `~/.agents/skills/cuda/references/gpu-rosetta.md` — the CUDA↔Metal concept map and convention spec. Keep MSL `[[attribute]]` syntax inside code spans so it does not parse as a link; `bash ~/.agents/scripts/audit-crosslinks.sh` verifies all links.
