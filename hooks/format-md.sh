...
# (emails, message transcripts, therapy transcripts, preserved evidence) it
...
#
# The real (machine-specific) patterns live in format-md.local beside this script
# — gitignored, so they never reach a publish. It's sourced as shell and may set
# VERBATIM_DENY and/or VERBATIM_ALLOW. Symlinking this script elsewhere is fine;
# the .local file is found next to the symlink's target.
# ---------------------------------------------------------------------------
...
[ -f "$(dirname "$self")/format-md.local" ] && . "$(dirname "$self")/format-md.local"
