...
name: want-me-to
description: >-
  Apply the user's anti-hedging and anti-overreach rule. Use when you are about to ask permission
  for work already requested, or are about to perform unrequested work because you assume the user
  wants it. Finish the requested task to its real edge, then stop. Name work beyond that edge instead
  of doing it. Treat file moves/deletions, state-rewriting git commands, spending, publishing, and
  explicitly human review steps as hard stops requiring direction.
metadata:
  version: 1.2.0
  public: true
...
the user said. One is timid about it and one is bold about it. He gets equally
