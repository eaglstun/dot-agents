...
| [[apple-metal:README]]                                   | **Rendering Metal** — stereo passes, depth effects, encoders/textures, for the karaoke-headset renderer                            | `metal-renderer` / `metal-fx-researcher` agents |
| [[cuda:README]]                                    | **NVIDIA CUDA** — the CT2 reference GPU backend the Metal backend mirrors                                                          | `cuda-references` agent                         |
| `~/.agents/skills/ffglitch/references/gpu-post.md` | **Video-pipeline Metal** — glitchgpu compute CLI, VideoToolbox, Resolve DCTL (skill-local; links out to shelves, nothing links in) | ffglitch skill                                  |
...
- **Syntax:** a shelf is a skill; `[[shelf:file]]` resolves to
  `~/.agents/skills/<shelf>/references/<file>.md`. Bare `[[file]]` = same shelf
  only; from any other shelf, link this hub as `[[cuda:gpu-rosetta]]`.
...
- **Direction:** global shelves may link to each other and to this rosetta.
  Non-shelf reference files (e.g. ffglitch's, project notes) may link OUT to
  shelves but nothing links INTO them (shelves never depend on a project existing).
- **Don't force bidirectionality** at write time; `~/.agents/scripts/audit-crosslinks.sh` reports dangling links and asymmetric pairs for periodic cleanup.
- **Namespace boundary:** bare `[[name]]` outside a shelf's `references/` is someone
  else's convention (the user's memory wikilinks use the same syntax) — the auditor
  judges namespaced links everywhere, bare ones only inside shelves.
