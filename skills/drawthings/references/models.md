...
than trusting whatever the UI last had. (This bites `scripts/gen-concepts.py` in the
ericeaglstun-ai repo, which hardcodes 22.)
...
**Face / identity:** **`ericoo_1500_lora_diffusers_lora_f16.ckpt`** (143 MB, FLUX.1,
rank-16) — the user's face. Registry name `ericoo (1500)`, **trigger `ooeric man`**. Source
`.safetensors` is kept at
`~/Documents/dev/oo-band/gen/identity/eric/dataset/ericoo_1500_lora_diffusers.safetensors`
(git-ignored). Works headlessly via `--lora`; at scale 1.0 it pulls output toward
casual-snapshot styling, so drop to ~0.7 when the prompt calls for a strong art direction.

**Other:** `autoxxx_500_500_lora_f32.ckpt` (a custom training run),
`gonzalomoxlfluxpony_v30fluxdaio_lora_f16.ckpt`, `fuxcapacitynsfwporn_62bf16_lora_f16.ckpt`.
The latter two are ~4 KB **companion stubs** for base finetunes that do exist on disk —
they look broken in a registry audit but are intentional; leave them.

> **Corrected 2026-07-26.** This section previously listed `ericoo_2000_lora_f32.ckpt`
> (the trained the user face LoRA). **That file is no longer on disk** — the API rejects it
> with `Missing file: ericoo_2000_lora_f32.ckpt`, and it appears nowhere under
> `~/Library/Containers/com.liuliu.draw-things/`. (`custom_lora.json` lives at
> `Data/Documents/**Models**/custom_lora.json`, not `Data/Documents/`.) Verify a LoRA
> filename against
> `find ~/Library/Containers/com.liuliu.draw-things -iname "*lora*.ckpt"` before
> relying on this list. The training data survives — see the retrain note below.

**Retraining the the user face LoRA:** the curated, captioned dataset lives at
`~/Documents/dev/oo-band/gen/identity/eric/dataset/` (~19 jpg + matching `.txt` caption
pairs, angle-labelled: `001_closeup_neutral`, `002_three_quarter`, `003_profile_left`,
`004_slight_smile`, …). A rawer 10-image set is at `~/Documents/AI/lora-train/eric/`.
Prefer the oo-band set — it's already captioned and angle-balanced. See
[cli.md](cli.md) for `train_lora.py`.
