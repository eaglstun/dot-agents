...
Cross-shelf: [[cuda:gpu-rosetta]] (repo root) maps every Metal concept here to its CUDA twin file.
...
- **Crosslinks to sibling shelves:** link as `[[shelf:file]]` (e.g. `[[cuda:cublas-gemm]]`)
  ...
  concept map + full convention spec is `~/.agents/skills/cuda/references/gpu-rosetta.md`; check it
  before adding a GPU topic. MSL `[[attribute]]` syntax must stay inside code spans so it
  doesn't parse as a link. `bash ~/.agents/scripts/audit-crosslinks.sh` verifies.
