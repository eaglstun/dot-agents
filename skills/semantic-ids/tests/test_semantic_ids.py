...
class SemanticIDTests(unittest.TestCase):
    ...
    def test_codex_skill_metadata_is_read_and_updated(self):
        frontmatter = """name: example
description: Example skill.
metadata:
  version: 1.0.0
  semantic_id: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
  related_ids:
    - "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"
"""
        fields = semantic_ids.parse_fm(frontmatter, "yaml")
        self.assertEqual(fields["semantic_id"], "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")

        updated = semantic_ids.set_fields(
            frontmatter,
            {
                "semantic_id": ['semantic_id: "CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC"'],
                "topic_id": ['topic_id: "v2:ABCD"'],
                "related_ids": [
                    "related_ids:",
        ...
        self.assertIn('  semantic_id: "CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC"', updated)
        self.assertIn('  topic_id: "v2:ABCD"', updated)
        self.assertIn('    - "DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD"', updated)
        self.assertNotIn("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB", updated)
        self.assertNotIn("\nsemantic_id:", updated)
