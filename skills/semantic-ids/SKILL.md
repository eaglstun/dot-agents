---
name: semantic-ids
description: 'Semantic IDs — a 192-bit base64url string whose bits ARE the meaning of a document, so "what''s
  related to this" is an XOR and a popcount with no vector database. One engine, many corpora: ~/.agents
  (every skill, agent, and reference file) is one context; any project can define its own. Use when routing
  a request to the right skill/agent/reference by meaning rather than by grepping descriptions, when finding
  what''s semantically near a file, when adding semantic IDs to a new corpus or project, when a `semantic_id`
  looks wrong or two obviously-related documents score as unrelated, or when anything proposes changing
  the encoder or bit layout. Read this BEFORE editing a semantic_id by hand — several innocent-looking
  edits silently corrupt distances with no error.'
metadata:
  version: 2.0.0
  public: true
