...
def parse_fm(fm_text: str, dialect: str) -> dict:
    ...
    # Codex skills reserve top-level frontmatter for the Agent Skills schema.
    # Project-specific fields therefore live under `metadata:`. Flatten the
    # scalar/list values here so existing semantic-ID callers can read either
    # the legacy top-level form or the Codex-compatible nested form.
    if dialect == "yaml":
        metadata_start = next(
            (n for n, line in enumerate(lines) if line.rstrip() == "metadata:"),
            None,
        )
        if metadata_start is not None:
            ...
            for line in lines[metadata_start + 1 :]:
                if line and not line.startswith((" ", "\t")):
                    break
                nested.append(line[2:] if line.startswith("  ") else line.lstrip("\t"))
            fields.update(parse_fm("\n".join(nested), "yaml"))
    return fields
...
def set_fields(
    ...
    *,
    ...
    """
    lines = fm_text.splitlines()
    if under_metadata and dialect == "yaml":
        try:
            metadata_start = lines.index("metadata:")
        except ValueError:
            lines.append("metadata:")
            metadata_start = len(lines) - 1
        ...
        for key, new_lines in updates.items():
            ...
            rendered = [f"  {line}" for line in new_lines]
            for i in range(metadata_start + 1, metadata_end):
                if not pat.match(lines[i]):
                    continue
                j = i + 1
                sibling = re.compile(r"^  [A-Za-z_][A-Za-z0-9_-]*\s*:")
                while j < metadata_end and not sibling.match(lines[j]):
                    j += 1
                lines[i:j] = rendered
                metadata_end += len(rendered) - (j - i)
                break
            else:
                lines[metadata_end:metadata_end] = rendered
                metadata_end += len(rendered)
        return "\n".join(lines)
...
def cmd_mint(cfg: dict, args) -> None:
    ...
    for d in docs:
        ...
        fm = set_fields(
            ...
            updates,
            dialect,
            under_metadata=d["path"].name == "SKILL.md" and dialect == "yaml",
        )
        want = f"{fence}\n{fm}\n{fence}\n{d['body']}"
