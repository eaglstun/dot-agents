---
name: touchdesigner
description: 'TouchDesigner reference — the operator families (TOP/CHOP/SOP/MAT/DAT/COMP/POP) and how
  data crosses between them, the Python API (op(), me, parent(), par access, custom parameters, extensions,
  storage), the callback DATs, cooking and GPU performance, and project/file workflow (.toe, .tox, externalized
  scripts, toeexpand, version control). Also covers the touchdesigner MCP bridge for driving a LIVE session
  — building networks with execute_python_script, reading real parameter names, and seeing renders via
  get_top_image, and point clouds / depth capture (PLY sequences, Record3D, the 2025 POP family). Use
  when writing or debugging TouchDesigner Python, wiring a network, choosing which operator does a job,
  diagnosing a slow or non-cooking network, setting up a .toe project for git, or when the MCP bridge
  won''t connect. Also covers this machine''s install: 2025.33070 on Apple Silicon macOS, where Vulkan/MoltenVK
  replaces the NVIDIA/CUDA-only operator set.'
metadata:
  version: 1.1.0
  public: true
  semantic_id: F6-pI0kqUJVt-Vl_7PalyvasfO8okAAI
  related_ids:
  - LI07MXMzQp_N2StDZ36nq6atCO-z8AAE
