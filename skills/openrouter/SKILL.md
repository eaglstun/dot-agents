---
name: openrouter
description: Direct calls to OpenRouter's OpenAI-compatible API for chat/text completions across hundreds
  of models from one key — OpenAI, Anthropic, Google, Meta, DeepSeek, Mistral, Qwen, and more. Use for
  raw LLM output (drafting, classification, translation, fast chat) when you want one endpoint, automatic
  provider fallback, or a model not available through other skills.
metadata:
  version: 1.0.0
  public: true
