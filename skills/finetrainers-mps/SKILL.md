...
description: Apple Silicon (MPS) lane of the user's finetrainers fork. Use when training or debugging
  finetrainers on a Mac, touching the MPS guards or single-device Accelerate path, running/extending the
  CPU↔MPS parity tests, or hitting device/decode errors on macOS (decord, torchcodec, NCCL, LOCAL_RANK).
metadata:
  version: 1.0.0
  public: true
